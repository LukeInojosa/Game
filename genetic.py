...
class Genetic:
        ...
        while not dead_end:
            ...
            if len(free_directions) > 0:
                ...
                if len(free_directions) > 1:
                    wrong_decision = {"current_position": current_position,"new_move":new_move}
                current_position = self.labyrinth.move_index(current_position,new_move)
                new_gene.append(new_move)
            else:
                dead_end = True
        new_gene.append(5)
        return {"gene": new_gene , "fitness": float("inf"),"knowledge": [wrong_decision]}
    
    def initialize_population(self):
        for i in range(self.len_population):
            self.population.append(self.initialize_gene(self.labyrinth.start,[]))
        self.max_len = len(max(self.population,key=lambda x: len(x["gene"]))["gene"])
    ...
    def mutation(self,gene,probability = 0.4):
        ...
        if random.random() <= probability:
            ...
            gene = self.initialize_gene(choice["position"],gene[:choice["i"]])["gene"]
        return gene

    def cross_over(self,gene1,gene2):
        cross = self.cross_points(gene1,gene2)
        if len(cross) > 0:
